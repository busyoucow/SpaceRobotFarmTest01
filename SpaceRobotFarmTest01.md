...
現在はアップデートが行われており、基板レイアウト変更や表面実装部品等の事前実装が済んだ状態になっておりその組み立ては
...
電子部品や電装品は以前執筆した記事(https://github.com/busyoucow/robotWired)にて組み上げたため、今回は水の通るパイプをつなげてみた。
下の写真で左手で持っているのが栽培瓶側に入れる給排水パイプと水位センサーである。
...
またパイプを切る長さや配置も気を使い、長さが足りなくなったり長すぎて折れてしまわないよう(中心部に1本折れている)気を付けたい。
またポンプ給排水口には太いパイプが必要であるが、バルブにはできるだけ細いパイプを使うべきであるだろう。
